	 ...
	 <div id="middle">
       <jsp:include page="sidebar.jsp" flush="true"/>
		...
		<div id="center-column">
			<div class="top-bar">
				<form name=pageset action="chinese.jsp" method=post class="page" >
				...
				<div class="breadcrumbs">当前位置&gt;&gt;
				  <a href="../homepage.jsp">主页面</a> / 闪客江湖信息概览</div>
			</div><br />
...
<%
   ...
   if(chineseVector!=null) allrows = chineseVector.size();
   ...
   if(pageid<pagecount&&pageid>0){
      for(int i=0; i<pagerows;i++) {
        Chinese tempChinese = (Chinese)chineseVector.get(pagerows*(pageid-1)+i);
...
%>
	   ...
	   <%  }  %>
			...
			<td><a href="view_chinese.jsp?cid=<%=tempChinese.getID()%>"><font size="-1">
			    <%=tempChinese.getTitle()%></font></a>
			</td>
			
			<%
              String text = tempChinese.getDetail();
              if (text.length()>50) { 
            %>
			...
			<td><a href="edit_chinese.jsp?cid=<%=tempChinese.getID()%>">
	            <img src="../css/images/edit.png" title="编辑<%=tempChinese.getTitle()%>此栏目的属性"
	                 alt="编辑<%=tempChinese.getTitle()%>此栏目的属性" border="0"></a>
	        </td>
			<td><a href="CHINESEINFO?cmd=delChinese&cid=<%=tempChinese.getID()%>" onclick='if(window.confirm("操作不能恢复，您确定要删除此栏 目？")){return true;}else{return false;}'>
	            <img src="../css/images/rubbish.png" title="删除<%=tempChinese.getTitle()%>此栏目 "
	                 alt="删除<%=tempChinese.getTitle()%>此栏目" border="0"></a>
	        </td>
		 </tr>
...
<% 
   ...
   for(int i=0; i<chineseVector.size()-pagerows*(pagecount-1);i++) {
      Chinese tempChinese = (Chinese)chineseVector.get(pagerows*(pageid-1)+i);
      int num = pagerows*(pageid-1)+i+1;
      if ( num%2==0 ) {
%>
...
%>	
         ...
         <td align="right" colspan="8"><font size="-1">
           ...
           <% if (pageid>1){ %>
              <a href="chinese.jsp?pageid=<%=pageid-1%>">上一页</a> 
           <% }
           
             if(pageid<pagecount) { %>
              <a href="chinese.jsp?pageid=<%=pageid+1%>">下一页</a> 
           <% } %>
           
              &nbsp;转到第<SELECT onChange="javascript:window.location='chinese.jsp?pageid='+this.options[this.selectedIndex].value;" size=1 name=page>
	       <%	
	          int p = 1;
		      while( p<=pagecount ) {
			   if( p==pageid )
