...
public class ParagraphLayout extends ConstraintLayout {
	...
	public ParagraphLayout(int hMargin, int vMargin, int hGapMajor,
			int vGapMajor, int hGapMinor, int vGapMinor) {
	...
	public void measureLayout(Container target, Dimension dimension, int type) {
		...
		if (count > 0) {
			...
			if (dimension != null) {
				...
				int spareHeight = size.height - (y + rowHeight) - insets.top
				...
				for (int i = 0; i < count; i++) {
					...
					if (includeComponent(c)) {
						...
						if (i == 0 || n == NEW_PARAGRAPH || n == NEW_PARAGRAPH_TOP) {
							if (i != 0)
								layoutRow(target, sizes, start, i - 1, y, rowWidth, rowHeight,
										firstLine, type, paragraphType);
							stretchV = false;
							paragraphType = n;
							start = i;
							firstLine = true;
							...
							if (!lastWasParagraph) {
								layoutRow(target, sizes, start, count - 1, y, rowWidth,
										rowHeight, firstLine, type, paragraphType);
								stretchV = false;
								start = i;
								firstLine = false;
								y += rowHeight + vGapMinor;
				...
				layoutRow(target, sizes, start, count - 1, y, rowWidth, rowHeight,
						firstLine, type, paragraphType);
			}
		}
	...
	protected void layoutRow(Container target, Dimension[] sizes, int start,
			int end, int y, int rowWidth, int rowHeight, boolean paragraph, int type,
			Integer paragraphType) {
		int x = 0;
		Insets insets = target.getInsets();
		Dimension size = target.getSize();
		int spareWidth = size.width - rowWidth - insets.left - insets.right - 2
				* hMargin;
		...
		for (int i = start; i <= end; i++) {
			...
			if (includeComponent(c)) {
				...
				int yOffset = paragraphType == NEW_PARAGRAPH_TOP ? 0
						: (rowHeight - h) / 2;
				if (target.getComponentOrientation().isLeftToRight())
					...
					c.setBounds(
							size.width - insets.right - insets.left - hMargin - x - w,
							insets.top + vMargin + y + yOffset, w, h);
				x += w + hGapMinor;
			}
		}
	}
